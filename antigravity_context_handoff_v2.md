...
1. **Bounding-box padding**: DONE (logic implemented + unit-tested; pending a live crop run on the Windows box to regenerate crops/manifest). Added `pipeline/bbox_utils.py` `pad_box(box, page_width, page_height, padding=15)` which expands each side by a fixed 15px, clamped to page bounds, without mutating the input `box`. Returns **integer** coords suitable for `PIL.Image.crop` — top-left is `math.floor`ed, bottom-right is `math.ceil`ed, so padding always rounds outward to whole pixels (fixes a float→`crop()` TypeError/subpixel bug). Wired into `pipeline/crop_assets.py`: `box` stays the canonical detection coords, a new `padded_box` field is recorded in `manifest.json`, and the crop uses `padded_box`. Page dimensions are read dynamically from the live image (`img.width`/`img.height`), not hardcoded. Schema gained a `padded_box` field on `VisualAsset`. Verified by `pipeline/test_bbox_padding.py` (6/6: standard, top-left clamp, bottom-right clamp, `box` immutability, fractional coords, and a PIL crop smoke test at the production 1366x1899 render resolution — all asserting integer outputs).
